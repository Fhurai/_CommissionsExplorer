...
if ($artist !== null) {
	...
	foreach ($contents as $key => $art) {
		// Browsing the content of the folder.

		// Get the file extension.
		$extension = pathinfo($art, PATHINFO_EXTENSION);

		if (!file_exists("../../../thumbs/" . substr($art, 16)) && !in_array($extension, ["wav", "WAV", "mp3", "MP3", "mp4", "MP4", "mov", "MOV", "avi", "AVI", "txt", "TXT"])) {
